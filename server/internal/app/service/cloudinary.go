...
func (c *CloudinaryService) UploadImage(filename, path, cldFolder string) (string, error) {
	resp, err := c.cld.Upload.Upload(context.TODO(), path, uploader.UploadParams{Folder: cldFolder, Overwrite: true, PublicID: filename})
	if err != nil {
		return "", err
	}
	return resp.URL, nil
